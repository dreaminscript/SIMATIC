...
// Keamanan Tambahan: Cek status saat ini di database
$cek = mysqli_query($conn, "SELECT status FROM cuti WHERE id_cuti='$id'");
$data_cek = mysqli_fetch_assoc($cek);

if ($data_cek['status'] != 'Menunggu Dosen') {
    echo "<script>alert('Pengajuan ini sudah diproses dan tidak dapat ditolak.'); window.location='dashboard.php';</script>";
    exit;
}
...
if (isset($_POST['submit_tolak'])) {
    ...
    // Update hanya jika statusnya masih 'Menunggu Dosen'
    $query = mysqli_query($conn, "UPDATE cuti SET status='Ditolak Dosen', catatan='$catatan' WHERE id_cuti='$id_post' AND status='Menunggu Dosen'");
            ...
            <div class="card-body p-4">
                <form action="" method="POST" id="formTolak">
                    ...
                    <div class="d-flex justify-content-end gap-2 action-btns">
                        <a href="detail.php?id=<?= htmlspecialchars($id); ?>" class="btn btn-secondary fw-bold px-4">Batalkan</a>
